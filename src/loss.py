...
class CustomCriterion:
    ...
    def __init__(self, samples_per_cls, device, fp16=False, loss_type="softmax"):
        ...
        if loss_type == "softmax":
            ...
            tau = 1.0
            self.logit_adj_val = torch.tensor(tau * np.log(self.frequency_per_cls)).float().to(self.device)
            self.logit_adj_val = self.logit_adj_val.half() if fp16 else self.logit_adj_val.float()
